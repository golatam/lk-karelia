...
(function (factory) {
  ...
  var insertCSS = function insertCSS(cssText) {
    ...
    elem.attr("type", 'text/css'); //$("head").append(elem);
...
/***/ "./resources/js/libs/timepicker.js":
/*!*****************************************!*\
  !*** ./resources/js/libs/timepicker.js ***!
  \*****************************************/
/***/ ((module, exports, __webpack_require__) => {
...
(function (factory) {
  ...
  $.extend(Timepicker.prototype, {
    ...
    */
    _newInst: function _newInst($input, opts) {
      ...
      tp_inst.support = detectSupport(tp_inst._defaults.timeFormat + (tp_inst._defaults.pickerTimeFormat ? tp_inst._defaults.pickerTimeFormat : '') + (tp_inst._defaults.altTimeFormat ? tp_inst._defaults.altTimeFormat : '')); // controlType is string - key to our this._controls
      ...
      tp_inst._defaults.timezoneList = timezoneList; // set the default units
    ...
    */
    _injectTimePicker: function _injectTimePicker() {
      var $dp = this.inst.dpDiv,
          ...
          size = null,
          i = 0,
          l = 0; // Prevent displaying twice

      if ($dp.find("div.ui-timepicker-div").length === 0 && o.showTimepicker) {
        var noDisplay = ' ui_tpicker_unit_hide',
            html = '<div class="ui-timepicker-div' + (o.isRTL ? ' ui-timepicker-rtl' : '') + (o.oneLine && o.controlType === 'select' ? ' ui-timepicker-oneLine' : '') + '"><dl>' + '<dt class="ui_tpicker_time_label' + (o.showTime ? '' : noDisplay) + '">' + o.timeText + '</dt>' + '<dd class="ui_tpicker_time ' + (o.showTime ? '' : noDisplay) + '"><input class="ui_tpicker_time_input" ' + (o.timeInput ? '' : 'disabled') + '/></dd>'; // Create the markup

        for (i = 0, l = this.units.length; i < l; i++) {
          litem = this.units[i];
          uitem = litem.substr(0, 1).toUpperCase() + litem.substr(1);
          show = o['show' + uitem] !== null ? o['show' + uitem] : this.support[litem]; // Added by Peter Medeiros:
        ...
        html += '<dd class="ui_tpicker_timezone' + (showTz ? '' : noDisplay) + '"></dd>'; // Create the elements from string

        html += '</dl></div>';
        var $tp = $(html); // if we only want time picker...
        ...
        for (i = 0, l = tp_inst.units.length; i < l; i++) {
          ...
          uitem = litem.substr(0, 1).toUpperCase() + litem.substr(1);
          show = o['show' + uitem] !== null ? o['show' + uitem] : this.support[litem]; // add the slider

          tp_inst[litem + '_slider'] = tp_inst.control.create(tp_inst, $tp.find('.ui_tpicker_' + litem + '_slider'), litem, tp_inst[litem], o[litem + 'Min'], max[litem], o['step' + uitem]); // adjust the grid and add click event
        ...
        if (this._defaults.addSliderAccess) {
          ...
          setTimeout(function () {
            ...
            if ($tp.find('.ui-slider-access').length === 0) {
              $tp.find('.ui-slider:visible').sliderAccess(sliderAccessArgs); // fix any grids since sliders are shorter
    ...
    */
    _onTimeChange: function _onTimeChange() {
      ...
      var ampm = o[hour < 12 ? 'amNames' : 'pmNames'][0]; // If the update was done in the input field, the input field should not be updated.
      ...
      if (hasChanged) {
        this._updateDateTime(); //this.$input.focus(); // may automatically open the picker on setDate
    ...
    */
    _updateDateTime: function _updateDateTime(dp_inst) {
      ...
      var formattedDateTime = this.formattedDate; // if a slider was changed but datepicker doesn't have a value yet, set it
  ...
  */
  ...
  $.datepicker.parseTime = function (timeFormat, timeString, options) {
    var o = extendRemove(extendRemove({}, $.timepicker._defaults), options || {}),
        iso8601 = timeFormat.replace(/\'.*?\'/g, '').indexOf('Z') !== -1; // Strict parse requires the timeString to match the timeFormat exactly
  ...
  */
  ...
  $.datepicker._selectDate = function (id, dateStr) {
    ...
    if (tp_inst && inst.settings.showTimepicker) {
      ...
      inst.inline = inst.stay_open = true; //This way the onSelect handler called from calendarpicker get the full dateTime
  ...
  */
  ...
  $.datepicker._setTime = function (inst, date) {
    ...
    if (tp_inst) {
      var defaults = tp_inst._defaults; // calling _setTime with no date sets time to defaults
  ...
  $.datepicker._getDateDatepicker = function (target, noDefault) {
    ...
    if (tp_inst) {
      ...
      if (date && tp_inst._parseTime(currDT, !inst.settings.timeOnly)) {
        ...
        date.setMicroseconds(tp_inst.microsec); // This is important if you are using the timezone option, javascript's Date
        // object will only return the timezone offset for the current locale, so we
        // adjust it accordingly.  If not using timezone option this won't matter..
  ...
  */
  ...
  $.timepicker.handleRange = function (method, startTime, endTime, options) {
    ...
    }, options); // for the mean time this fixes an issue with calling getDate with timepicker()
...
/* harmony import */ var jquery_ui_ui_widgets_slider__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! jquery-ui/ui/widgets/slider */ "./node_modules/jquery-ui/ui/widgets/slider.js");
/* harmony import */ var jquery_ui_ui_widgets_slider__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(jquery_ui_ui_widgets_slider__WEBPACK_IMPORTED_MODULE_4__);
...
/* harmony import */ var jquery_ui_ui_i18n_datepicker_ru__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! jquery-ui/ui/i18n/datepicker-ru */ "./node_modules/jquery-ui/ui/i18n/datepicker-ru.js");
/* harmony import */ var jquery_ui_ui_i18n_datepicker_ru__WEBPACK_IMPORTED_MODULE_6___default = /*#__PURE__*/__webpack_require__.n(jquery_ui_ui_i18n_datepicker_ru__WEBPACK_IMPORTED_MODULE_6__);
/* harmony import */ var jquery_ui_ui_widgets_sortable_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! jquery-ui/ui/widgets/sortable.js */ "./node_modules/jquery-ui/ui/widgets/sortable.js");
/* harmony import */ var jquery_ui_ui_widgets_sortable_js__WEBPACK_IMPORTED_MODULE_7___default = /*#__PURE__*/__webpack_require__.n(jquery_ui_ui_widgets_sortable_js__WEBPACK_IMPORTED_MODULE_7__);
/* harmony import */ var _libs_timepicker__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./libs/timepicker */ "./resources/js/libs/timepicker.js");
/* harmony import */ var _libs_timepicker__WEBPACK_IMPORTED_MODULE_8___default = /*#__PURE__*/__webpack_require__.n(_libs_timepicker__WEBPACK_IMPORTED_MODULE_8__);
/* harmony import */ var select2_dist_js_select2_full_min__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! select2/dist/js/select2.full.min */ "./node_modules/select2/dist/js/select2.full.min.js");
/* harmony import */ var select2_dist_js_select2_full_min__WEBPACK_IMPORTED_MODULE_9___default = /*#__PURE__*/__webpack_require__.n(select2_dist_js_select2_full_min__WEBPACK_IMPORTED_MODULE_9__);
/* harmony import */ var _libs_notify__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./libs/notify */ "./resources/js/libs/notify.js");
/* harmony import */ var _libs_notify__WEBPACK_IMPORTED_MODULE_10___default = /*#__PURE__*/__webpack_require__.n(_libs_notify__WEBPACK_IMPORTED_MODULE_10__);
/* harmony import */ var micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! micromodal/dist/micromodal */ "./node_modules/micromodal/dist/micromodal.js");
/* harmony import */ var micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11___default = /*#__PURE__*/__webpack_require__.n(micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11__);
/* harmony import */ var vanilla_masker_build_vanilla_masker_min__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! vanilla-masker/build/vanilla-masker.min */ "./node_modules/vanilla-masker/build/vanilla-masker.min.js");
/* harmony import */ var vanilla_masker_build_vanilla_masker_min__WEBPACK_IMPORTED_MODULE_12___default = /*#__PURE__*/__webpack_require__.n(vanilla_masker_build_vanilla_masker_min__WEBPACK_IMPORTED_MODULE_12__);
...
(jquery__WEBPACK_IMPORTED_MODULE_0___default().timepicker.regional.ru) = {
...
jquery__WEBPACK_IMPORTED_MODULE_0___default().timepicker.setDefaults((jquery__WEBPACK_IMPORTED_MODULE_0___default().timepicker.regional.ru)); // Touch Slider UI
...
(function ($) {
  ...
  micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11___default().init(); // Double scroll
  ...
  $('.js-delete-item').on('click', function () {
    ...
    micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11___default().show('js-remove-modal');
  }); // Delete items
  ...
  $('.js-submit-delete-form').on('click', function () {
    ...
    micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11___default().close('js-remove-modal');
    $.ajax({
      url: action,
      type: 'DELETE',
      data: data,
  ...
  if ($('.form__tinymce').length > 0) {
    tinymce.init({
      ...
      language: 'ru',
      convert_urls: false,
      plugins: ["advlist autolink link image lists charmap print preview hr anchor pagebreak media", "searchreplace wordcount visualblocks visualchars insertdatetime media nonbreaking", "table directionality emoticons paste responsivefilemanager code"],
      ...
      external_plugins: {
        "filemanager": "/assets/plugins/filemanager/plugin.min.js"
      },
      image_class_list: [{
        title: 'None',
        value: ''
        ...
        title: 'По центру',
        value: 'content-images-2'
      }, {
        title: 'Справа',
        value: 'content-images-3'
      }, {
        title: 'Слева с превью',
        value: 'content-images-1 glightbox'
      }, {
        title: 'По центру с превью',
        value: 'content-images-2 glightbox'
      }, {
        title: 'Справа с превью',
        value: 'content-images-3 glightbox'
      }]
    });
  } // Air Datepicker
        ...
        success: function success(result) {
          if (result.data.length) {
            ...
            result.data.forEach(function (item) {
              $html += "<a href=\"/".concat(result.entity, "/").concat(item.id).concat(result.type, "\" class=\"searching__result-item\">");

              for (var k in item) {
                if (k !== 'type_id') {
                  $html += "<span>".concat(item[k], "</span>");
                }

                if (result.types && result.types[item[k]]) {
                  $html += "<span>".concat(result.types[item[k]], "</span>");
                }
              }
  ...
  $('.js-restore-item').on('click', function () {
    $(this).closest('tr').find('.js-check').prop('checked', true);
    micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11___default().show('js-restore-modal');
  ...
  $('.js-submit-restore-form').on('click', function () {
    ...
    micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11___default().close('js-restore-modal');
    $.ajax({
      url: action,
      type: 'post',
      data: data,
  ...
  $(document).on('click', '.js-sort-btn', function (e) {
    ...
    sorting = JSON.parse(get_cookie(cookieName, '[]')); // console.log(sortColumn)
  ...
  if (vanillaMasker) {
    vanilla_masker_build_vanilla_masker_min__WEBPACK_IMPORTED_MODULE_12___default()(vanillaMasker).maskPattern("9:99:99:999999999999");
  }
...
})(jQuery); // Добавление множества файлов


function files() {
  // Добавление множества фото
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('change', '.js-upload-files', function (e) {
    ...
    if (!!accept && accept.length) {
      accept = accept.split(',').map(function (accept) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().each($this[0].files, function (i, file) {
      if (!file.name.length) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default().notify("Файл отсутствует!", "error");
        return false;
      }
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().each(data, function (k, i) {
      ajaxData.append(k, i);
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      success: function success(result) {
        if (result.success) {
          jquery__WEBPACK_IMPORTED_MODULE_0___default().each(result.files, function (k, i) {
            html = '<div class="row row--small row--ai-center">' + '<div class="col-xs-9 col-sm-10 col-md-11">' + '<div class="form__element form__element--large">' + '<a href="' + i.path + '" class="btn btn--full btn--text-left btn--medium btn--gray" download>' + '<i class="fas fa-file-' + i.extension + '"></i>' + '<span class="btn__text btn__text--right">' + i.name + '</span>' + '</a>' + '</div>' + '</div>' + '<div class="col-xs-3 col-sm-2 col-md-1">' + '<button type="button" class="btn btn--medium btn--orange btn--default-square js-remove-file-item" data-model-id="' + result.id + '" data-morph-class="' + result.morphClass + '" data-file-id="' + i.id + '">' + '<i class="fas fa-trash-alt"></i>' + '</button>' + '</div>' + '</div>';
            filesBlock.append(html);
          });
          jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(result.message, "success");
        } else {
          jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(result.message, "error");
        }
      },
      error: function error(result) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(result.message, "error");
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-remove-file-item', function (e) {
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
...
function image() {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('change', '.js-upload-image', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        ajaxData = new FormData(),
      ...
      success: function success(result) {
        if (result.success) {
          ...
          if (result.imageHeader) {
            jquery__WEBPACK_IMPORTED_MODULE_0___default()('.header__profile-photo').attr('src', result.imageFilePath);
          }
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('change', '.js-upload-image-crop', function (e) {
    var file = this.files;
    ...
    if (file.length > 0) {
      ...
      fileReader.onload = function (event) {
        var $img = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.js-image-crop');
    ...
    micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11___default().show('js-image-crop-modal');
  }); // Crop image & send ajax

  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-crop-btn', function () {
    var $image = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.js-image-crop');
    $image.cropper("getCroppedCanvas").toBlob(function (blob) {
      // FormData is a built-in javascript object
      var formData = new FormData(),
          formImage = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.form__image'),
          data = formImage.data();
      jquery__WEBPACK_IMPORTED_MODULE_0___default().each(data, function (k, i) {
        formData.append(k, i);
      });
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
        ...
        success: function success(result) {
          if (result.success) {
            ...
            jquery__WEBPACK_IMPORTED_MODULE_0___default()('.js-image-crop').cropper('destroy');
            jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(result.message, "success");
            micromodal_dist_micromodal__WEBPACK_IMPORTED_MODULE_11___default().close('js-image-crop-modal');
          } else {
            formImage.find('.form__image-loader').removeClass('show');
            jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(result.message, "error");
          }
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-remove-image', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        formImage = $this.closest('.form__image'),
        inputFile = formImage.find('.js-upload-image'),
        ajaxData = formImage.data(),
        headerProfilePhoto = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.header__profile-photo');
    formImage.find('.form__image-loader').addClass('show');
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(".js-recommended-autocomplete").autocomplete({
    minLength: 2,
    ...
    select: function select(e, ui) {
      var recommendedItems = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).closest('.panel__content').find('.js-recommended-items'),
          html,
      ...
      if (ui.item.success) {
        ...
        recommendedItems.append(html); // Присоединяем выбраную статью
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
          ...
          if (response.success) {
            jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(response.message, "success");
          } else {
            jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(response.message, "error");
          }
        }).fail(function (jqXHR, textStatus) {});
      } else {
        jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(ui.item.message, "error");
      }
    },
    change: function change(e, ui) {}
  });
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-delete-recommended-item', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        data = {
    ...
    $this.closest('.js-recommended-item').remove(); // Отсоединяем выбраную статью
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-set-filter', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        data = $this.data(); // Отсоединяем выбраную статью
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-notify-questionnaires', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        data = $this.data(); // Отсоединяем выбраную статью
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-add-to-forum', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        formGroup = $this.closest('.form__group'),
        data = formGroup.data();
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-activate-on-forum', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        formGroup = $this.closest('.form__group'),
        data = formGroup.data();
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-deactivate-on-forum', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        formGroup = $this.closest('.form__group'),
        data = formGroup.data();
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-send-credentials-from-forum', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        formGroup = $this.closest('.form__group'),
        data = formGroup.data();
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('change', '.js-template-change-city', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      if (response.success) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default().each(response.venuesList, function (id, value) {
          html += '<option value="' + id + '" >' + value + '</option>';
        });
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('.js-template-block-address').html(html);
        jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(response.message, "success");
      } else {
        jquery__WEBPACK_IMPORTED_MODULE_0___default().notify(response.message, "error");
      }
...
function images() {
  // Добавление множества фото
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('change', '.js-upload-images', function (e) {
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
        ajaxData = new FormData(),
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      success: function success(result) {
        if (result.success) {
          ...
          jquery__WEBPACK_IMPORTED_MODULE_0___default().each(result.images, function (k, i) {
            ...
            html = '<div class="col-xs-6 col-sm-2 js-images-item ' + $this.data('group') + '" data-position="' + i.position + '" data-id="' + result.id + '" data-image-id="' + i.id + '" data-morph-class="' + result.morphClass + '">' + '<div class="form__image form__image--fluid">' + '<button class="form__image-delete js-remove-images">Удалить</button>' + '<img src="' + i.path + '" alt="" class="form__image-img">' + '</div>' + description + '</div>';
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on('click', '.js-remove-images', function (e) {
    e.preventDefault();
    var $this = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this),
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.js-change-position-images').sortable({
    tolerance: 'pointer',
    update: function update(e, ui) {
      var imagesList = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.js-images-list'),
          btnBlock = imagesList.find('.js-btn-block'),
          ajaxData = {},
          images = {};
      jquery__WEBPACK_IMPORTED_MODULE_0___default().each(imagesList.find('.js-images-item.' + btnBlock.data('group')), function (k, i) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()(i).data('position', k);
        images[k] = {
          'imageId': jquery__WEBPACK_IMPORTED_MODULE_0___default()(i).data('imageId')
        };
      });
...
function saveDraftData() {
  var form = jquery__WEBPACK_IMPORTED_MODULE_0___default()('form.js-save-draft-data'),
  ...
  if (!!url) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      error: function error(result) {// $.notify(result.message, "error");
      }
    });
  }
}
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkdback"] = self["webpackChunkdback"] || [];
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["assets/css/app"], () => (__webpack_require__("./resources/sass/main.scss")))
