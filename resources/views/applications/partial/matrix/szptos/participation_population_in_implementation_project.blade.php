...
@else
<div class="form__group-wrap js-group-items">
    @for($i=0;$i<$matrixKeysShow->count();$i++)
    <div class="row row--small js-group-item">
        @foreach($fields as $field)
        <div class="col-xs-12 col-sm-10">
            <div class="form__group form__group--input">
                <input name="{{ $fieldName }}[{{ $field }}][]" class="form__input form__input--medium" value="{{ optional($models)[$i]?->{$field} }}" type="text" placeholder="@lang("{$entity}_applications.matrix.{$fieldName}.{$field}")">
            </div>
        </div>
        @endforeach
    </div>
    @endfor
    @foreach($models->diffKeys($matrixKeysShow) as $key => $item)
    <div class="row row--small js-group-item">
        @foreach($fields as $field)
        <div class="col-xs-12 col-sm-10">
            <div class="form__group form__group--input">
