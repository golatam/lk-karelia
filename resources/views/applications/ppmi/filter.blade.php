<div id="filter" class="panel__filter panel__content hidden">
    <form action="{{ route("applications.{$entity}.filter") }}" method="post" enctype="multipart/form-data" class="form" id="filter-form">
        ...
        <div class="row form__group-wrap">
            ...
            <div class="col-xs-12 col-sm-4">
                <div class="form__group form__group--input">
                    <label class="form__label form__label--medium" for="contest_id">@lang("{$entity}_applications.contest_id")</label>
                    <select name="contest_id" id="contest_id" class="form__select form__select--medium">
                        <option value="" >Выбрать</option>
                        @foreach($contests as $contestKey => $contestName)
                            <option value="{{ $contestKey }}" @if ((int) $filter['contest_id'] === (int) $contestKey){{ 'selected' }}@endif>{{ $contestName }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
