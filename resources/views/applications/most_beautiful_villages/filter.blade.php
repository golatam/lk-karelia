<div id="filter" class="panel__filter panel__content hidden">
    <form action="{{ route("applications.{$entity}.filter") }}" method="post" enctype="multipart/form-data" class="form" id="filter-form">
        ...
        <div class="row form__group-wrap">
            ...
            <div class="col-xs-12 col-sm-4">
                <div class="form__group form__group--input">
                    <label class="form__label form__label--medium" for="settlement_id">@lang("{$entity}_applications.settlement_id")</label>
                    <select name="settlement_id" id="settlement_id" class="form__select form__select--medium">
                        <option value="" >Выбрать</option>
                        @foreach($settlements as $settlementId => $settlementName)
                        <option value="{{ $settlementId }}" @if ((int) $filter['settlement_id'] === (int) $settlementId){{ 'selected' }}@endif>{{ $settlementName }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
